...
export async function processChat(ctx: ChatContext, input: ChatInput): Promise<ChatOutput> {
  ...
  if (input.contentType === "audio" && input.audioUrl) {
    try {
      ...
      await db.insert(caiusVoiceInteractions).values({
        ...
        language: "pt-BR",
        status: "done",
        requiresHumanReview: transcription.text.length < 10,
        createdAt: new Date(),
      });
      ...
      await logCaiusAudit({
        ...
        channel: ctx.channel,
        nup: ctx.nup,
        inputSummary: `Áudio transcrito: "${transcription.text.substring(0, 100)}"`,
        metadata: { audioUrl: input.audioUrl, duration: transcription.duration },
      });
    } catch (err) {
  ...
  try {
    ...
    llmResponse = result.content ?? "Desculpe, não consegui processar sua solicitação no momento.";
...
Forneça um resumo claro e objetivo com: situação atual, histórico resumido e próximos passos recomendados.`;
  ...
  try {
    ...
    return result.content ?? "Não foi possível gerar o resumo.";
  } catch {
